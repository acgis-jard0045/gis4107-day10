...
def test_dumpFilelist():
    dump.dumpFileList("D:\\acgis\\gis4107_Intro2Prog\\day10\\lab\\SamJandAlisonO\\TestFolder","one.txt")
    expected = open('two.txt', 'r').read()
    actual = FileUtils.getFileContent(os.path.join(script_folder,"one.txt"))
    if expected == actual:
        print "the file names are correct"
    else:
        print "the file names are incorrect"
